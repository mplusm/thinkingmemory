...
def init_db():
    ...
    # Create the `memory` table HASH-partitioned by tenant_id (raw DDL, since
    # SQLModel can't express partitioning). create_all then skips it and creates
    # the remaining tables (memory_audit, memory_edge).
...
def create_memory_partitioned(n: int | None = None) -> None:
    """Create the `memory` table partitioned by HASH(tenant_id), if absent.

    Mirrors ``engine.models.Memory``. The PK is (id, tenant_id) because a
    partitioned table's primary key must include the partition key; ``id`` stays
    an identity column. No-op if `memory` already exists.
    """
    from thinkingmemory.core.embeddings import EMBEDDING_DIM

    settings = get_settings()
    parts = n or settings.memory_partitions
    engine = get_engine()
    with engine.begin() as conn:
        exists = conn.execute(
            text("SELECT to_regclass('public.memory')")
        ).scalar()
        if exists:
            return
        conn.execute(
            text(
                ...
                CREATE TABLE memory (
                    ...
                    mtype varchar NOT NULL DEFAULT 'episodic',
        ...
        for r in range(parts):
            conn.execute(
                text(
                    f"CREATE TABLE memory_p{r} PARTITION OF memory "
                    f"FOR VALUES WITH (MODULUS {parts}, REMAINDER {r})"
                )
            )
    logger.info("Created partitioned `memory` table with %d hash partitions", parts)
